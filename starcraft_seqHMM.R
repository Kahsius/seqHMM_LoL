...
if(!MODE_PER_TEAM) {
  print("--- Mise en forme et sommage, mode : global")
  ...
  print("--- Mise en forme et sommage, mode : par équipe")
  ...
  mmsum <- matrix(0, nrow = N_TEAM*d[1], ncol = d[2])
  for(j in 1:N_TEAM){
    for(i in 1:n_matches){
      mmsum[1:N_STATES+(j-1)*N_STATES,] <- mmsum[1:N_STATES+(j-1)*N_STATES,]+m_all_forward[[j]][[i]]
    }    
  }
...
# Nettoyage
rm(seq.each.team, range, range2, v, index, tpm, rho, init, r,
   m, m_forward, i, j, k, l, rn, cn, m_all_forward, mmm, mmm2,
   mtmp, seuil, mmsum, mmsum2, h, co, h2, co2, segments, seqdef.mmm, model, d)
print("All done")
